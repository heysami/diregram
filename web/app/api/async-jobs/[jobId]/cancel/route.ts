...
import { getAsyncJobById, requestAsyncJobCancel } from '@/lib/server/async-jobs/repo';
...
function extractJobId(request: Request): string {
  ...
  const m = pathname.match(/\/api\/async-jobs\/([^/]+)\/cancel$/);
...
function canCancelJob(auth: Awaited<ReturnType<typeof resolveAsyncJobRequestAuth>>, job: AsyncJobRow): boolean {
