...
export function CreateTestFileModal({
  flowRoots,
  flowRefs,
  onClose,
  onCreate,
  ...
  onClose: () => void;
  onCreate: (res: { name: string; flowRootId: string; flowNodeId: string }) => void;
}) {
    ...
    <div className="fixed inset-0 z-[90] bg-black/40 flex items-center justify-center">
      <div className="w-[92vw] max-w-3xl overflow-hidden mac-window">
        <div className="mac-titlebar">
          <div className="mac-title">Create test file</div>
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input className="mac-field w-full h-9" value={name} onChange={(e) => setName(e.target.value)} placeholder="Test name" />

            <div className="mt-4 text-xs font-semibold text-slate-700 mb-2">Flow</div>
            <select className="mac-field w-full h-9" value={flowRootId} onChange={(e) => setFlowRootId(e.target.value)} disabled={flowRoots.length === 0}>
              {flowRoots.map((r) => (
          ...
          <div>
              ...
              {flowNodeOptions.length === 0 ? (
                ...
                flowNodeOptions.map((o) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 ${flowNodeId === o.id ? 'bg-blue-50' : ''} ${
            ...
            {!flowRefs[flowNodeId] ? <div className="mt-2 text-[11px] text-red-600">Pick a node that has a Flow→Process reference.</div> : null}
          </div>
        </div>

        <div className="border-t px-4 py-3 flex items-center justify-end gap-2">
          <button type="button" onClick={onClose} className="mac-btn h-8">
            Cancel
          </button>
          <button
            type="button"
            onClick={() => {
              if (!canCreate) return;
              onCreate({ name: name.trim(), flowRootId, flowNodeId });
            }}
            disabled={!canCreate}
