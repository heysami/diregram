...
export function paintUrl(kind: 'fill' | 'stroke', sid: string) {
  return `url(#${sid}__${kind})`;
}
...
function stopsToDefs(stops: GradientStop[]) {
  ...
  return arr.map((s, i) => (
    <stop
      ...
      stopColor={String((s as any)?.color || '#000000')}
    />
  ));
}
...
export function getPatternDef(kind: PatternKind, a: string, b: string, id: string) {
  if (kind === 'dots') {
    return (
      <pattern id={id} patternUnits="userSpaceOnUse" width="10" height="10">
        ...
        <circle cx="2.5" cy="2.5" r="1.6" fill={a} />
        <circle cx="7.5" cy="7.5" r="1.6" fill={a} />
      </pattern>
    );
  }
  if (kind === 'checker') {
    return (
      <pattern id={id} patternUnits="userSpaceOnUse" width="12" height="12">
        ...
        <rect x="0" y="0" width="6" height="6" fill={a} />
        <rect x="6" y="6" width="6" height="6" fill={a} />
      </pattern>
    );
  }
  // stripes
  return (
    <pattern id={id} patternUnits="userSpaceOnUse" width="10" height="10" patternTransform="rotate(45)">
      <rect width="10" height="10" fill={b} />
      <rect x="0" y="0" width="4" height="10" fill={a} />
    </pattern>
  );
}
...
export function getPaintDefs(opts: {
  ...
  const id = `${opts.sid}__${opts.kind}`;
  if (opts.mode === 'linear') {
