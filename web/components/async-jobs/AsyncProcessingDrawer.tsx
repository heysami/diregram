...
function getPrimaryDiagramFileId(result: Record<string, unknown>): string {
  const a = String(result.primaryDiagramFileId || '').trim();
  if (a) return a;
  return String(result.singleDiagramFileId || '').trim();
}
        ...
        {open ? (
          <div className="max-h-[420px] overflow-auto p-2 space-y-2">
            ...
            {jobs.map((j) => {
              ...
              const isProjectPipeline = String(j.kind || '') === 'project_pipeline';
              const primaryDiagramFileId = getPrimaryDiagramFileId(j.result);
              return (
                <div key={j.id} className="mac-double-outline p-2 text-xs space-y-1">
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    ...
                    {isProjectPipeline && j.status === 'succeeded' && primaryDiagramFileId ? (
                      <button
                        type="button"
                        className="mac-btn h-7"
                        onClick={() => {
                          if (typeof window === 'undefined') return;
                          window.location.assign(`/editor?file=${encodeURIComponent(primaryDiagramFileId)}`);
                        }}
                        title="Open primary diagram generated by the pipeline"
                      >
